        ...
        actions: [
          CupertinoDialogAction(
            onPressed: () {
              ...
              if (n.isEmpty) return;
              final renamed = _scheme.copyWith(name: n);
              setState(() => _scheme = renamed);
              // Persist immediately so the schemes list shows the new name
              StorageService.instance
                  .saveScheme(renamed.copyWith(grid: _grid))
                  .then((_) {
                    if (mounted) setState(() => _dirty = false);
                    widget.onSchemeOpened?.call(renamed);
                  });
            },
...
class _TopBar extends StatelessWidget {
  final String     name;
  final bool       dirty;
  final VoidCallback onNew, onSave, onBirds, onRename;
