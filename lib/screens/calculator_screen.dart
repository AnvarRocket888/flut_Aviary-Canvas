...
class _CalculatorScreenState extends State<CalculatorScreen> {
  ...
  bool   _ran             = false;
  AviaryScheme?     _linkedScheme;

  // Load bird counts and area from a saved scheme
  Future<void> _loadFromScheme() async {
    final schemes = await StorageService.instance.loadAllSchemes();
    if (!mounted || schemes.isEmpty) return;
    AviaryScheme? chosen;
    await showCupertinoModalPopup(
      context: context,
      builder: (_) => CupertinoActionSheet(
        title:   const Text('Load from Scheme'),
        message: const Text('Pre-fill bird counts from a saved scheme'),
        actions: schemes.map((s) => CupertinoActionSheetAction(
          onPressed: () { chosen = s; Navigator.pop(context); },
          child: Column(
            mainAxisSize: MainAxisSize.min,
            children: [
              Text(s.name, style: AppTextStyles.body),
              Text('${s.totalBirds} birds · ${s.totalAreaM2.toStringAsFixed(1)} m²',
    ...
    setState(() {
      _linkedScheme = chosen;
      _birdCounts
        ..clear()
        ..addAll(chosen!.birdCounts);
      _totalAreaM2 = chosen!.totalAreaM2.clamp(1, 200);
      final zones  = chosen!.zoneCellCounts;
      _feederCells = (zones[1] ?? _feederCells).clamp(0, 30);
      _waterCells  = (zones[2] ?? _waterCells).clamp(0,  20);
      _nestCells   = (zones[3] ?? _nestCells ).clamp(0,  20);
      _perchCells  = (zones[4] ?? _perchCells).clamp(0,  30);
      _result      = null;
      _ran         = false;
    });
  }

  // Save calculation results as PDF
  Future<void> _savePdf() async {
    ...
    doc.addPage(
      pw.Page(
        ...
        build: (ctx) => pw.Column(
          ...
          children: [
            pw.Text('Aviary Calculator Results',
                style: pw.TextStyle(fontSize: 22, fontWeight: pw.FontWeight.bold)),
            if (_linkedScheme != null)
              pw.Text('Scheme: ${_linkedScheme!.name}',
                  style: const pw.TextStyle(fontSize: 13, color: PdfColors.grey700)),
            pw.SizedBox(height: 16),
            pw.Divider(),
            pw.SizedBox(height: 8),
            _pdfRow('Space check',   r.space),
            _pdfRow('Feeders',       r.feeders),
            _pdfRow('Water',         r.water),
            _pdfRow('Nesting',       r.nesting),
            _pdfRow('Perch',         r.perch),
            if (r.conflicts.isNotEmpty) ...[
              pw.SizedBox(height: 12),
              pw.Text('Conflicts',
                  style: pw.TextStyle(fontSize: 14, fontWeight: pw.FontWeight.bold,
                      color: PdfColors.red700)),
              ...r.conflicts.map((c) => pw.Padding(
                padding: const pw.EdgeInsets.only(top: 4),
                child:   pw.Text('• ${c.message}',
            ...
            pw.SizedBox(height: 8),
            pw.Text('Total area: ${_totalAreaM2.toStringAsFixed(1)} m²'),
            pw.SizedBox(height: 24),
            pw.Text('Generated by Aviary Canvas',
    ...
    try {
      ...
      final file  = File('${dir.path}/aviary_calc_${DateTime.now().millisecondsSinceEpoch}.pdf');
      await file.writeAsBytes(bytes);
      await Share.shareXFiles([XFile(file.path)], text: 'Aviary Calculation Results');
    } catch (_) {
      if (!mounted) return;
      // ignore share errors (user cancelled sheet)
    }
  }

  pw.Widget _pdfRow(String label, CalcResult res) => pw.Padding(
    padding: const pw.EdgeInsets.only(bottom: 6),
    child: pw.Row(
      children: [
        pw.Text(res.isOk ? '✅' : '❌', style: const pw.TextStyle(fontSize: 12)),
        pw.SizedBox(width: 6),
        pw.Expanded(
          child: pw.Column(
            crossAxisAlignment: pw.CrossAxisAlignment.start,
            children: [
              pw.Text(label,
                  style: pw.TextStyle(
                    ...
                    color: res.isOk ? PdfColors.green700 : PdfColors.red700,
                  )),
              pw.Text(res.message, style: const pw.TextStyle(fontSize: 10)),
              if (res.recommendation != null)
                pw.Text('Tip: ${res.recommendation}',
              ...
              child: CupertinoButton(
                ...
                child: Container(
                  ...
                  child: Row(
                    ...
                    children: [
                      const Icon(CupertinoIcons.share, color: AppColors.accent, size: 18),
                      const SizedBox(width: 8),
                      Text('Save Results as PDF',
